...
import { canManageOperations, canViewOrderBilling } from '@/lib/auth/capabilities'
...
import { billingModelLabel, evidenceFrequencyLabel, serviceProviderLabel, timeTrackingModeLabel } from '@/modules/orders/labels'
...
export default function OrderDetailPage() {
  ...
  const canManageOrder = canManageOperations(user.role)
  const canSeeBilling = canViewOrderBilling(user.role)
  const currentEmployee = store.employees.find((item) => item.email.toLowerCase() === user.email.toLowerCase())
  const employeeAssigned = user.role !== 'employee' || Boolean(currentEmployee && store.orderAssignmentRules.some((rule) => rule.active && rule.orderId === params.id && rule.personId === currentEmployee.id))
  ...
  if (!order) {
    return <section className="page"><div className="detail-back-row"><Link className="text-link" href="/orders">← Aufträge</Link></div><EmptyState title="Auftrag nicht gefunden" description="Der Auftrag ist nicht mehr vorhanden oder wurde noch nicht geladen." /></section>
  }

  if (!employeeAssigned) {
    return <section className="page"><div className="detail-back-row"><Link className="text-link" href="/orders">← Aufträge</Link></div><EmptyState title="Kein Zugriff" description="Dieser Auftrag ist dir nicht zugewiesen." /></section>
  }
  ...
  return (
    <section className="page">
      ...
      <div className="page-title"><div><p className="eyebrow">AUFTRAG</p><h1>{order.name}</h1><p className="page-description">{order.customerName}{order.endCustomerName ? ` · Endkunde: ${order.endCustomerName}` : ''}</p></div><div className="page-action-group"><StatusBadge status={order.status} />{canManageOrder && <button className="button secondary" onClick={() => setEditOpen(true)}><Icon name="edit" size={15}/> Bearbeiten</button>}</div></div>

      <div className="order-kpi-strip">
        <span><small>Budget</small><strong>{order.budgetHours} h</strong></span>
        <span><small>Verbraucht</small><strong>{metrics.usedHours} h</strong></span>
        <span><small>Rest</small><strong>{metrics.remainingHours} h</strong></span>
      </div>
      ...
      <div className={sectionOpen ? 'object-hub-layout section-open' : 'object-hub-layout'}>
        <nav className="object-hub-nav" aria-label="Auftragsbereiche">
          ...
          {canManageOrder && <HubButton active={activeSection === 'people'} label="Team" meta={`${assignments.length} zugewiesen`} onClick={() => { setActiveSection('people'); setSectionOpen(true) } } />}
          <HubButton active={activeSection === 'time'} label="Zeiterfassung" meta={policy?.timeTracking.evidence.required ? `${evidenceFrequencyLabel(policy.timeTracking.evidence.frequency)} · Nachweis Pflicht` : 'Auftragsregeln'} onClick={() => { setActiveSection('time'); setSectionOpen(true) } } />
          ...
          {canSeeBilling && <HubButton active={activeSection === 'billing'} label="Abrechnung" meta={billingModelLabel(order.billingModel)} onClick={() => { setActiveSection('billing'); setSectionOpen(true) } } />}
        ...
        <div className="object-hub-content">
          ...
          {activeSection === 'overview' && <section className="hub-panel">
            ...
            <div className="summary-list">
              ...
              <SummaryRow label="Referenz" value={order.mandateRef || '–'} />
              <SummaryRow label="Beschaffungsreferenz" value={order.procurementRef || '–'} />
              <SummaryRow label="Abrechnungsmodell" value={billingModelLabel(order.billingModel)} />
            </div>
          ...
          {activeSection === 'time' && <section className="hub-panel">
            <div className="section-title"><div><h2>Zeiterfassung</h2><p>Aktive Regeln dieses Auftrags.</p></div>{canManageOrder && <button className="button secondary compact-action" onClick={() => setPolicyOpen(true)}><Icon name="edit" size={15}/> Regeln</button>}</div>
            {policy ? <div className="summary-list"><SummaryRow label="Erfassung" value={timeTrackingModeLabel(policy.timeTracking.mode)} /><SummaryRow label="Rundung" value={`${policy.timeTracking.bookingIntervalMinutes} Minuten`} /><SummaryRow label="Beschreibung" value={policy.timeTracking.requireDescription ? 'Pflicht' : 'Optional'} /><SummaryRow label="Nachweis" value={policy.timeTracking.evidence.required ? `${evidenceFrequencyLabel(policy.timeTracking.evidence.frequency)} · Pflicht` : 'Nicht erforderlich'} /><SummaryRow label="Freigabe" value={policy.timeTracking.evidence.blockApprovalWhenMissing ? 'Nachweis muss vorhanden sein' : 'Ohne Nachweis möglich'} /><SummaryRow label="Fakturierung" value={policy.timeTracking.evidence.blockBillingWhenMissing ? 'Bei fehlendem Nachweis gesperrt' : 'Nicht blockiert'} /></div> : <EmptyState title="Standardregeln aktiv" description="Für diesen Auftrag sind keine zusätzlichen Zeitregeln definiert." />}
          </section>}

          {canManageOrder && activeSection === 'people' && <section className="hub-panel">
            ...
            {assignments.length ? <div className="hub-row-list">{assignments.map((assignment) => { const effective = policy ? resolveTimeTrackingPolicy(policy, assignment) : assignment.timePolicyOverride as TimeTrackingPolicy | undefined; return <InteractiveRow className="hub-row interactive-hub-row" key={`${assignment.orderId}-${assignment.personId}`} onActivate={() => setAssignmentOpen(assignment)} ariaLabel={`${personName(assignment.personId, store)} Regel bearbeiten`}><span><strong>{personName(assignment.personId, store)}</strong><small>{serviceProviderLabel(assignment.providerType)} · {effective?.evidence.required ? `${evidenceFrequencyLabel(effective.evidence.frequency)} Nachweis` : 'Auftragsstandard'}</small></span><Icon name="chevron" size={15}/></InteractiveRow>})}</div> : <EmptyState title="Noch niemand zugewiesen" description="Füge Mitarbeitende oder externe Leistungserbringer hinzu." />}
          </section>}

          {canSeeBilling && activeSection === 'billing' && <section className="hub-panel">
            <div className="section-title"><div><h2>Abrechnung</h2><p>Nur abrechnungsrelevante Regeln und Werte.</p></div></div>
            {policy ? <div className="summary-list"><SummaryRow label="Modell" value={policy.billing.model} /><SummaryRow label="Rechnungsdarstellung" value={policy.billing.invoiceGrouping} /><SummaryRow label="Stundenauszug" value={policy.billing.attachTimesheet ? 'Anhängen' : 'Nicht anhängen'} /><SummaryRow label="PO / Bestellnummer" value={policy.billing.purchaseOrderRequired ? 'Pflicht' : 'Optional'} /><SummaryRow label="Leistungsperiode" value={policy.billing.servicePeriodRequired ? 'Pflicht' : 'Optional'} /><SummaryRow label="Freigabe ab" value={policy.approval.invoiceApprovalThreshold ? formatChf(policy.approval.invoiceApprovalThreshold) : 'Keine Schwelle'} /></div> : <div className="summary-list"><SummaryRow label="Modell" value={order.billingModel}/><SummaryRow label="Verkaufssatz" value={`${formatChf(order.salesRate)}/h`}/><SummaryRow label="Budget" value={`${order.budgetHours} h`}/></div>}
            <div className="hub-inline-action"><Link href={`/invoices?new=1&order=${order.id}`} className="text-link">Rechnung erstellen <Icon name="chevron" size={14}/></Link></div>
          </section>}

          {activeSection === 'evidence' && <section className="hub-panel">
            <div className="section-title"><div><h2>Nachweise</h2><p>Rapporte und Freigaben für diesen Auftrag.</p></div><button className="button secondary compact-action" onClick={() => setUploadOpen(true)}><Icon name="plus" size={15}/> Nachweis erfassen</button></div>
            <div className="hub-row-list">{evidence.length ? evidence.map((item) => <div className="hub-row evidence-hub-row" key={item.id}><span><strong>{item.fileName}</strong><small>{formatDate(item.periodDate)} · {personName(item.personId, store)}</small></span><StatusBadge status={item.status} />{canManageOrder && item.status !== 'verified' && <button className="text-button" onClick={() => { store.updateEvidence(item.id, { status: 'verified', verifiedAt: new Date().toISOString() }); feedback.success('Nachweis geprüft.') }}>Prüfen</button>}</div>) : <EmptyState title="Keine Nachweise vorhanden" description="Für diesen Auftrag wurden noch keine Rapporte hochgeladen." />}</div>
          </section>}
          ...
          {activeSection === 'documents' && <section className="hub-panel">
            ...
            <div className="hub-row-list">
              ...
              <button type="button" className="hub-row hub-row-button" onClick={() => setUploadOpen(true)}><span><strong>Zeitnachweis erfassen</strong><small>PDF auswählen und Prüfstatus dokumentieren</small></span><Icon name="chevron" size={15}/></button>
            </div>
          </section>}
        </div>
      </div>

      {canManageOrder && editOpen && <OrderEditor order={order} onClose={() => setEditOpen(false)} />}
      {uploadOpen && <EvidenceUpload orderId={order.id} onClose={() => setUploadOpen(false)} />}
      {canManageOrder && policyOpen && <PolicyEditor orderId={order.id} current={policy ?? createDefaultOrderPolicy(order.id, order.billingModel)} onClose={() => setPolicyOpen(false)} />}
      {canManageOrder && assignmentOpen && <AssignmentEditor orderId={order.id} current={assignmentOpen === 'new' ? null : assignmentOpen} onClose={() => setAssignmentOpen(null)} />}
    </section>
  )
  ...
  function EvidenceUpload({ orderId, onClose }: { orderId: string; onClose: () => void }) {
    ...
    function save(event: React.FormEvent) {
      ...
      feedback.success('Zeitnachweis gespeichert.')
      onClose()
    }

    return <StandardFormSheet open title={<>Zeitnachweis erfassen</>} description={<>Die Datei wird aktuell nicht serverseitig gespeichert. Dateiname und Prüfstatus werden dokumentiert.</>} onClose={onClose} onSubmit={save} formId="id-page-sheet-2" footer={orderEntries.length ? <><button type="button" className="button secondary" onClick={onClose}>Abbrechen</button><button type="submit" form="id-page-sheet-2" className="button primary">Nachweis speichern</button></> : null}>{orderEntries.length ? <><div className="form-grid"><label className="full"><span>Zeiteintrag *</span><Select aria-label="Zeiteintrag" value={timeEntryId} onChange={(e) => setTimeEntryId(e.target.value)} required>{orderEntries.map((entry) => <option key={entry.id} value={entry.id}>{formatDate(entry.date)} · {entry.personName} · {entry.hours} h</option>)}</Select></label><label className="full"><span>PDF-Nachweis *</span><Input type="file" accept="application/pdf" onChange={(e) => setFile(e.target.files?.[0] ?? null)} required/></label><div className="form-toggle-field"><span>Signiert</span><Toggle label="Signiert" checked={signed} onChange={setSigned}/></div><div className="form-toggle-field"><span>Kunde bestätigt</span><Toggle label="Kunde bestätigt" checked={customerApproved} onChange={setCustomerApproved}/></div></div></> : <EmptyState title="Noch keine Zeiten vorhanden" description="Erfasse zuerst eine Zeit auf diesem Auftrag." action={<Link href={`/time?new=1&order=${orderId}`} className="button primary">Zeit erfassen</Link>} />}</StandardFormSheet>
  }
