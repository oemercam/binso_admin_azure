...
export default function OrderDetailPage() {
  ...
  return (
    <section className="page">
      ...
      <div className="page-title"><div><p className="eyebrow">AUFTRAG</p><h1>{order.name}</h1><p className="page-description">{order.customerName}{order.endCustomerName ? ` · Endkunde: ${order.endCustomerName}` : ''}</p></div><div className="page-action-group"><StatusBadge status={order.status} /><button className="button secondary" onClick={() => setEditOpen(true)}><Icon name="edit" size={15}/> Bearbeiten</button></div></div>
      ...
      <div className={sectionOpen ? 'object-hub-layout section-open' : 'object-hub-layout'}>
        ...
        <div className="object-hub-content">
          ...
          {activeSection === 'evidence' && <section className="hub-panel">
            ...
            <div className="hub-row-list">{evidence.length ? evidence.map((item) => <div className="hub-row evidence-hub-row" key={item.id}><span><strong>{item.fileName}</strong><small>{item.periodDate} · {personName(item.personId, store)}</small></span><StatusBadge status={item.status} label={evidenceLabel(item.status)} />{item.status !== 'verified' && <button className="text-button" onClick={() => store.updateEvidence(item.id, { status: 'verified', verifiedAt: new Date().toISOString() })}>Prüfen</button>}</div>) : <div className="empty-state"><strong>Keine Nachweise vorhanden</strong><span>Für diesen Auftrag wurden noch keine Rapporte hochgeladen.</span></div>}</div>
          </section>}
