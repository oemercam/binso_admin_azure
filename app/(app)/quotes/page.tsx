...
export default function QuotesPage() {
  ...
  const filteredQuotes = useMemo(() => { const q = query.trim().toLocaleLowerCase('de-CH'); return q ? store.quotes.filter((quote) => `${quote.number} ${quote.title} ${quote.customerName}`.toLocaleLowerCase('de-CH').includes(q)) : store.quotes }, [query, store.quotes])
  ...
  return <section className="page">
    ...
    <div className="module-toolbar"><SearchField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Angebote durchsuchen" aria-label="Angebote durchsuchen"/><span className="toolbar-meta">{filteredQuotes.length} Angebote</span></div><div className="data-list compact-overview-list">
    ...
    {sending && <QuoteSend quote={sending} onClose={() => setSending(null)} onSent={(quote) => { setSending(null); setPreview(quote); feedback.success('Angebot als versendet markiert.') }} />}
  </section>
    ...
    return <StandardFormSheet open title={<>Angebot erstellen</>} description={<>Für einen Entwurf reichen Kunde, Titel, Gültigkeit und Positionen.</>} onClose={onClose} onSubmit={save} formId="quotes-page-sheet-1" footer={<><button type="button" className="button secondary" onClick={onClose}>Abbrechen</button><button type="submit" form="quotes-page-sheet-1" className="button primary">Entwurf erstellen</button></>} mode="fullscreen">
      <details className="edit-step" open><summary><span><strong>1 · Grundlage</strong><small>Kunde, Titel und Gültigkeit</small></span><Icon name="chevron" size={15}/></summary><div className="edit-step-body">
        <div className="form-grid">
          <label className="full"><span>Kunde / Interessent *</span><Select searchable searchPlaceholder="Kunden durchsuchen" value={customerId} onChange={(e) => setCustomerId(e.target.value)} required>{selectableCustomers.map((c) => <option key={c.id} value={c.id}>{c.name}{c.status === 'prospect' ? ' · Interessent' : ''}</option>)}</Select></label>
