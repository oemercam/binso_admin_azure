...
export default function QuotesPage() {
  ...
  function QuoteSend({ quote, onClose, onSent }: { quote: Quote; onClose: () => void; onSent: (q: Quote) => void }) {
    ...
    return <div className="overlay-layer sheet-layer" onMouseDown={onClose}><form className="form-sheet compact-sheet bottom-sheet standard-mobile-sheet" onSubmit={send} onMouseDown={(e) => e.stopPropagation()}><div className="sheet-grabber"/><div className="sheet-heading"><div><strong>Angebot versenden</strong><span>PDF-Vorschau entspricht dem Dokument im Anhang.</span></div><CloseButton onClick={onClose} /></div><div className="send-meta"><span><small>Von</small><strong>{from || 'Nicht konfiguriert'}</strong></span>{store.appSettings.mail.financeCc && <span><small>CC</small><strong>{store.appSettings.mail.financeCc}</strong></span>}</div><div className="form-grid"><label className="full"><span>Empfänger *</span><input type="email" value={to} onChange={(e) => setTo(e.target.value)} required/></label><label className="full"><span>Betreff *</span><input value={subject} onChange={(e) => setSubject(e.target.value)} required/></label><label className="full"><span>Nachricht *</span><textarea rows={8} value={body} onChange={(e) => setBody(e.target.value)} required/></label></div><div className="demo-hint">Demo-Versand: Status und Empfänger werden gespeichert. Der echte Versand über {from || 'die konfigurierte Absenderadresse'} benötigt noch Microsoft Graph und serverseitige PDF-Erzeugung.</div><div className="sheet-actions"><button type="button" className="button secondary" onClick={onClose}>Abbrechen</button><button className="button primary" disabled={!from}><Icon name="send" size={15}/> Senden</button></div></form></div>
  }
}
