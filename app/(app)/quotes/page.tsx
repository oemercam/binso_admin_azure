...
export default function QuotesPage() {
  ...
  return <section className="page">
    ...
    <div className="data-list">
      ...
      {store.quotes.map((quote) => <div className="data-row quote-grid" key={quote.id}><span className="primary-cell"><strong>{quote.number} · {quote.title}</strong><small>Version {quote.version} · {quote.lines.length} Positionen</small></span><span>{quote.customerName}</span><span>{quote.validUntil}</span><span>{chf.format(quote.amount)}</span><span className={`status ${quote.status}`}>{labels[quote.status]}</span><button className="row-link" onClick={() => setPreview(quote)} aria-label={`${quote.number} öffnen`}><Icon name="chevron" size={15}/></button></div>)}
    </div>

    {preview && <div className="overlay-layer document-overlay" onMouseDown={() => setPreview(null)}><div className="document-preview-shell" onMouseDown={(e) => e.stopPropagation()}><div className="preview-toolbar"><div><strong>{preview.number} · Version {preview.version}</strong><span>{preview.customerName}</span></div><div><button className="icon-button" onClick={() => window.print()} title="PDF / Drucken"><Icon name="download" size={16}/></button><button className="icon-button" onClick={() => setPreview(null)}><Icon name="close" size={16}/></button></div></div>{missing(preview).length > 0 && <div className="document-warning"><strong>Noch nicht versandbereit</strong><span>Fehlend: {missing(preview).join(', ')}</span></div>}<DocumentPreviewFrame><BusinessDocument type="quote" company={store.companyProfile} customer={customerFor(preview)} quote={preview}/></DocumentPreviewFrame><div className="preview-actions"><button className="button secondary" onClick={() => setEditing(preview)}><Icon name="edit" size={15}/> Bearbeiten</button><button className="button secondary" disabled={missing(preview).length > 0} onClick={() => setSending(preview)}><Icon name="send" size={15}/> {preview.status === 'sent' ? 'Erneut senden' : 'Senden'}</button>{preview.status !== 'accepted' && <button className="button secondary" onClick={() => setStatus('accepted')}><Icon name="check" size={15}/> Annehmen</button>}{preview.status !== 'declined' && preview.status !== 'accepted' && <button className="button secondary" onClick={() => setStatus('declined')}>Ablehnen</button>}{preview.status === 'accepted' && <button className="button primary" onClick={orderFromQuote}><Icon name="orders" size={15}/> Auftrag erstellen</button>}<button className="button secondary" onClick={createRevision}>Neue Version</button></div></div></div>}
  ...
  function QuoteForm({ onClose, onSave }: { onClose: () => void; onSave: (q: Quote) => void }) {
    ...
    return <div className="overlay-layer sheet-layer" onMouseDown={onClose}><form className="form-sheet document-editor mobile-fullscreen-sheet" onSubmit={save} onMouseDown={(e) => e.stopPropagation()}><div className="sheet-grabber"/><div className="sheet-heading"><div><strong>Angebot erstellen</strong><span>Pflichtfelder und Rechnungsadresse werden geprüft.</span></div><button type="button" className="icon-button" onClick={onClose}><Icon name="close" size={17}/></button></div><div className="form-grid"><label className="full"><span>Kunde *</span><select value={customerId} onChange={(e) => setCustomerId(e.target.value)} required>{store.customers.filter((c) => c.status === 'active').map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label className="full"><span>Titel *</span><input value={title} onChange={(e) => setTitle(e.target.value)} required/></label><label><span>Gültig bis *</span><input type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} required/></label><label><span>Referenz / PO</span><input value={reference} onChange={(e) => setReference(e.target.value)}/></label></div><div className="line-editor"><div className="line-editor-head"><strong>Positionen</strong><button type="button" className="text-button" onClick={() => setLines((current) => [...current, newLine()])}><Icon name="plus" size={14}/> Position</button></div>{lines.map((line) => <div className="line-editor-row quote-line-editor" key={line.id}><label><span>Beschreibung</span><input value={line.description} onChange={(e) => updateLine(line.id, { description: e.target.value })} required/></label><label><span>Menge</span><input type="number" min="0.01" step="0.25" value={line.quantity} onChange={(e) => updateLine(line.id, { quantity: Number(e.target.value) })}/></label><label><span>Einheit</span><select value={line.unit} onChange={(e) => updateLine(line.id, { unit: e.target.value as QuoteLine['unit'] })}><option value="h">h</option><option value="Tag">Tag</option><option value="pauschal">pauschal</option></select></label><label><span>Preis CHF</span><input type="number" min="0" step="0.05" value={line.unitPrice} onChange={(e) => updateLine(line.id, { unitPrice: Number(e.target.value) })}/></label><button type="button" className="line-remove" disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((item) => item.id !== line.id))}>×</button></div>)}</div><div className="sheet-actions"><button type="button" className="button secondary" onClick={onClose}>Abbrechen</button><button className="button primary">Entwurf erstellen</button></div></form></div>
  }
  ...
  function QuoteEdit({ quote, onClose, onSave }: { quote: Quote; onClose: () => void; onSave: (q: Quote) => void }) {
    ...
    return <div className="overlay-layer sheet-layer" onMouseDown={onClose}><form className="form-sheet document-editor mobile-fullscreen-sheet" onSubmit={save} onMouseDown={(e) => e.stopPropagation()}><div className="sheet-grabber"/><div className="sheet-heading"><div><strong>Angebot bearbeiten</strong><span>{quote.number} · Version {quote.version}</span></div><button type="button" className="icon-button" onClick={onClose}><Icon name="close" size={17}/></button></div><div className="form-grid"><label><span>Empfänger *</span><input value={draft.recipientName ?? ''} onChange={(e) => setDraft({ ...draft, recipientName: e.target.value })} required/></label><label><span>E-Mail *</span><input type="email" value={draft.recipientEmail ?? ''} onChange={(e) => setDraft({ ...draft, recipientEmail: e.target.value })} required/></label><label className="full"><span>Adresse *</span><input value={draft.recipientAddress ?? ''} onChange={(e) => setDraft({ ...draft, recipientAddress: e.target.value })} required/></label><label><span>PLZ *</span><input value={draft.recipientZip ?? ''} onChange={(e) => setDraft({ ...draft, recipientZip: e.target.value })} required/></label><label><span>Ort *</span><input value={draft.recipientCity ?? ''} onChange={(e) => setDraft({ ...draft, recipientCity: e.target.value })} required/></label><label><span>Gültig bis *</span><input type="date" value={draft.validUntil} onChange={(e) => setDraft({ ...draft, validUntil: e.target.value })} required/></label><label><span>Referenz / PO</span><input value={draft.reference ?? ''} onChange={(e) => setDraft({ ...draft, reference: e.target.value })}/></label><label className="full"><span>Einleitungstext</span><textarea rows={4} value={draft.introText ?? ''} onChange={(e) => setDraft({ ...draft, introText: e.target.value })}/></label></div><div className="line-editor"><div className="line-editor-head"><strong>Positionen</strong><button type="button" className="text-button" onClick={() => setDraft((cur) => ({ ...cur, lines: [...cur.lines, newLine()] }))}><Icon name="plus" size={14}/> Position</button></div>{draft.lines.map((line) => <div className="line-editor-row quote-line-editor" key={line.id}><label><span>Beschreibung</span><input value={line.description} onChange={(e) => updateLine(line.id, { description: e.target.value })} required/></label><label><span>Menge</span><input type="number" step="0.25" value={line.quantity} onChange={(e) => updateLine(line.id, { quantity: Number(e.target.value) })}/></label><label><span>Einheit</span><select value={line.unit} onChange={(e) => updateLine(line.id, { unit: e.target.value as QuoteLine['unit'] })}><option value="h">h</option><option value="Tag">Tag</option><option value="pauschal">pauschal</option></select></label><label><span>Preis CHF</span><input type="number" step="0.05" value={line.unitPrice} onChange={(e) => updateLine(line.id, { unitPrice: Number(e.target.value) })}/></label><button type="button" className="line-remove" disabled={draft.lines.length === 1} onClick={() => setDraft((cur) => ({ ...cur, lines: cur.lines.filter((item) => item.id !== line.id) }))}>×</button></div>)}</div><label className="block-field"><span>Schlusstext</span><textarea rows={4} value={draft.outroText ?? ''} onChange={(e) => setDraft({ ...draft, outroText: e.target.value })}/></label><div className="sheet-actions"><button type="button" className="button secondary" onClick={onClose}>Abbrechen</button><button className="button primary">Speichern</button></div></form></div>
  }
  ...
  function QuoteSend({ quote, onClose, onSent }: { quote: Quote; onClose: () => void; onSent: (q: Quote) => void }) {
    ...
    return <div className="overlay-layer sheet-layer" onMouseDown={onClose}><form className="form-sheet compact-sheet bottom-sheet" onSubmit={send} onMouseDown={(e) => e.stopPropagation()}><div className="sheet-grabber"/><div className="sheet-heading"><div><strong>Angebot versenden</strong><span>PDF-Vorschau entspricht dem Dokument im Anhang.</span></div><button type="button" className="icon-button" onClick={onClose}><Icon name="close" size={17}/></button></div><div className="send-meta"><span><small>Von</small><strong>{from || 'Nicht konfiguriert'}</strong></span>{store.appSettings.mail.financeCc && <span><small>CC</small><strong>{store.appSettings.mail.financeCc}</strong></span>}</div><div className="form-grid"><label className="full"><span>Empfänger *</span><input type="email" value={to} onChange={(e) => setTo(e.target.value)} required/></label><label className="full"><span>Betreff *</span><input value={subject} onChange={(e) => setSubject(e.target.value)} required/></label><label className="full"><span>Nachricht *</span><textarea rows={8} value={body} onChange={(e) => setBody(e.target.value)} required/></label></div><div className="demo-hint">Demo-Versand: Status und Empfänger werden gespeichert. Der echte Versand über {from || 'die konfigurierte Absenderadresse'} benötigt noch Microsoft Graph und serverseitige PDF-Erzeugung.</div><div className="sheet-actions"><button type="button" className="button secondary" onClick={onClose}>Abbrechen</button><button className="button primary" disabled={!from}><Icon name="send" size={15}/> Senden</button></div></form></div>
  }
}
