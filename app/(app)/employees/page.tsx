...
export default function EmployeesPage() {
  ...
  const filteredEmployees = useMemo(() => { const q = normalizeSearch(query); return q ? store.employees.filter((employee) => normalizeSearch(`${employee.name} ${employee.email} ${roleLabel(employee.role)}`).includes(q)) : store.employees }, [query, store.employees])
  ...
  return (
    <section className="page">
      <PageHeader eyebrow="TEAM" title="Mitarbeitende" description="Mitarbeitende, Rollen und Zeitstatus verwalten." action={<button className="button primary page-primary-action" onClick={() => setCreating(true)} aria-label="Mitarbeitende erfassen" title="Mitarbeitende erfassen"><Icon name="plus" size={16}/><span>Mitarbeitende erfassen</span></button>} />
      <div className="module-toolbar"><SearchField value={query} onValueChange={setQuery} placeholder="Mitarbeitende durchsuchen" aria-label="Mitarbeitende durchsuchen"/><span className="toolbar-meta">{filteredEmployees.length} Mitarbeitende</span></div><div className="data-list">
      ...
      {creating && <EmployeeForm onClose={() => setCreating(false)} onSave={(employee) => { store.addEmployee(employee); feedback.success('Mitarbeitende erfasst.'); setCreating(false); setEditing(employee) }} />}
      {editing && <EmployeeEditor employee={store.employees.find((item) => item.id === editing.id) ?? editing} onClose={() => setEditing(null)} onSave={(employee) => { store.updateEmployee(employee.id, employee); feedback.success('Mitarbeiterprofil gespeichert.'); setEditing(null) }} />}
    </section>
  )
}
