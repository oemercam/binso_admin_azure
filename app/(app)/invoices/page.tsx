...
export default function InvoicesPage() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return q ? store.invoices.filter((invoice) => `${invoice.number} ${invoice.customerName} ${invoice.orderName ?? ''}`.toLocaleLowerCase('de-CH').includes(q)) : store.invoices
  ...
  return (
    <section className="page">
      ...
      <div className="module-toolbar"><SearchField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechnungen durchsuchen" aria-label="Rechnungen durchsuchen"/><span className="toolbar-meta">{filteredInvoices.length} Rechnungen</span></div><div className="data-list compact-overview-list">
        ...
        {filteredInvoices.map((invoice) => (
          <InteractiveRow className="data-row invoice-grid compact-overview-row" key={invoice.id} onActivate={() => setPreview(invoice)} ariaLabel={`${invoice.number} öffnen`}>
            ...
            <span className="overview-desktop-cell">{invoice.customerName}</span><span className="overview-desktop-cell">{fmt(invoice.due)}</span><span className="overview-desktop-cell"><strong>{chf.format(invoice.amount)}</strong><small>{invoice.lines.length} Positionen</small></span><StatusBadge status={effectiveInvoiceStatus(invoice)} label={invoiceStatusLabel(effectiveInvoiceStatus(invoice))} className="overview-desktop-cell" />
      ...
      {builderOpen && (
        <ResponsiveOverlay open={builderOpen} mobile="fullscreen" desktop="dialog" title="Rechnung erstellen" description="Zeiten übernehmen oder freie Rechnungspositionen erfassen." onClose={() => setBuilderOpen(false)} footer={<SheetActions><button type="button" className="button secondary" onClick={() => setBuilderOpen(false)}>Abbrechen</button><button type="button" className="button primary" disabled={!selected.length && !extraLines.some((line) => line.description.trim())} onClick={createInvoice}>Entwurf erstellen</button></SheetActions>}>
            <details className="edit-step" open><summary><span><strong>1 · Quelle</strong><small>Kunde und Auftrag auswählen</small></span><Icon name="chevron" size={15}/></summary><div className="edit-step-body"><div className="form-grid"><label className="full"><span>Kunde *</span><Select searchable searchPlaceholder="Kunden durchsuchen" value={customerId} onChange={(e) => { setCustomerId(e.target.value); setOrderId(''); setSelected([]) }} required>{store.customers.filter((customer) => customer.status === 'active').map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</Select></label><label className="full"><span>Auftrag</span><Select searchable searchPlaceholder="Aufträge durchsuchen" value={orderId} onChange={(e) => { const next = e.target.value; setOrderId(next); setSelected([]); const order = store.orders.find((item) => item.id === next); if (order) setCustomerId(order.customerId) }}><option value="">Freie Rechnung ohne Auftrag</option>{store.orders.filter((order) => order.customerId === customerId).map((order) => <option key={order.id} value={order.id}>{order.name}</option>)}</Select></label></div></div></details>
