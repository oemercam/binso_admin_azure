...
export default async function Page({params}:{params:Promise<{organizationId:string}>}){
  await requirePlatformRole(...PLATFORM_ROLE_GROUPS.all)
  ...
  return <section className="page apple-page">
    ...
    <SettingsSection title="Übersicht" description="Kerninformationen des SaaS-Mandanten."><SettingsValueRow title="Owner" value={value(o.owner_name)} description={value(o.owner_email)}/><SettingsValueRow title="Billing" value={value(o.billing_provider)} description={`${value(o.subscription_status)} · CHF ${value(o.unit_amount_chf)}`}/><SettingsValueRow title="Letzte Aktivität" value={o.last_active_at?formatDateTime(o.last_active_at):'–'}/><SettingsValueRow title="Pilot" value={o.is_pilot_customer?'Ja':'Nein'} description={value(o.pilot_status)}/></SettingsSection>
    ...
    <SettingsSection title="Abo-Historie" description="Letzte Subscription-Ereignisse.">{d.subscriptionEvents.map(e=><SettingsValueRow key={e.id} title={e.event_type} value={formatDateTime(e.created_at)} description={e.detail??undefined}/>)}</SettingsSection>
    ...
    <SettingsSection title="Interne Notizen" description="Nur für Binso-Operatoren sichtbar.">{d.notes.map(n=><SettingsValueRow key={n.id} title={n.author_email} value={formatDateTime(n.created_at)} description={n.note}/>)}</SettingsSection>
    <PlatformCustomerNoteForm organizationId={organizationId}/>
    <SettingsSection title="Audit" description="Letzte Betreiberänderungen.">{d.audit.map(a=><SettingsValueRow key={a.id} title={a.action} value={formatDateTime(a.created_at)} description={`${a.actor_email}${a.detail?` · ${a.detail}`:''}`}/>)}</SettingsSection>
  </section>
}
