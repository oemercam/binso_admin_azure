...
export async function POST(request:Request){try{requireSameOrigin(request)}catch{return apiError(403,'invalid_origin','Ungültige Anfragequelle.')}const b=await readJsonBody<{articleId?:string;helpful?:boolean}>(request,2048).catch(()=>null);if(!b?.articleId||typeof b.helpful!=='boolean')return apiError(422,'validation','Feedback ist ungültig.');const s=await getSession();let organizationId: string|undefined;if(s){organizationId=(await findAccessibleMembership(s.user.id))?.organizationId}await recordHelpFeedback({articleId:b.articleId,organizationId,userId:s?.user.id,helpful:b.helpful});return apiJson({ok:true})}
