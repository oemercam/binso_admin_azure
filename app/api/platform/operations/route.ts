...
export async function GET() {
  ...
  try {
    const [jobs, mail, events, audit] = await Promise.all([
      query('select id,job_name,status,summary,started_at,completed_at from job_runs order by started_at desc limit 20'),
      query('select id,organization_id,kind,entity_id,status,attempts,last_error,created_at,updated_at from mail_outbox order by created_at desc limit 100'),
      query('select id,severity,area,code,request_id,created_at from application_events order by created_at desc limit 30'),
      query('select id,actor_email,action,tenant_id,detail,created_at from platform_audit_events order by created_at desc limit 30'),
    ])
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const result = await query("update mail_outbox set status='cancelled',updated_at=now() where id=$1 and status in ('queued','failed','uncertain') returning id", [body.id])
    if (!result.rowCount) return apiError(409, 'conflict', 'Versand kann nicht mehr abgebrochen werden.')
    await query("insert into platform_audit_events(actor_user_id,actor_email,action,detail) values ($1,$2,'mail.cancelled',$3)", [session.user.id, session.user.email, body.id])
