...
export async function GET(request:Request){
  const s=await getPlatformSession();if(!s||!canSupportPlatform(s.user.platformRole))return apiError(403,'forbidden','Keine Plattformberechtigung.')
...
export async function PATCH(request:Request){
  ...
  const s=await getPlatformSession();if(!s||!canSupportPlatform(s.user.platformRole))return apiError(403,'forbidden','Keine Plattformberechtigung.')
  const b=await readJsonBody<{caseId?:string;message?:string;internalNote?:string;status?:SupportCaseStatus;caseType?:SupportCaseType;classification?:SupportCaseClassification|null;priority?:SupportCasePriority;assignedToUserId?:string|null}>(request,PRODUCT_LIMITS.apiBodySupportBytes).catch(()=>null)
