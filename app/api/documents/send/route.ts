...
export async function POST(request: Request) {
  ...
  try {
    ...
    const permission = body.kind === 'quote' ? 'quotes.write' : 'invoices.write'
    const context = await resolveAuthorizedTenantContext(body.organizationId, permission)
    if (!context) return apiError(403, 'forbidden', 'Keine Versandberechtigung.')
