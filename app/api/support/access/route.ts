...
export async function POST(request: Request) {
  ...
  const body = await readJsonBody<{ organizationId?: string; reason?: string }>(request, PRODUCT_LIMITS.apiBodyStandardBytes).catch(() => null)
...
export async function DELETE(request: Request) {
  try { requireSameOrigin(request) } catch { return apiError(403, 'invalid_origin', 'Ungültige Anfragequelle.') }
  const body = await readJsonBody<{ organizationId?: string; grantId?: string }>(request, PRODUCT_LIMITS.apiBodyStandardBytes).catch(() => null)
