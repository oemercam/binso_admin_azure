...
import type { SupportCaseCategory } from '@/types/domain'
...
export async function POST(request:Request){
  ...
  const body=await readJsonBody<{category?:SupportCaseCategory;subject?:string;message?:string;currentPage?:string;entityType?:string;entityId?:string;buildVersion?:string;browser?:string}>(request,32_768).catch(()=>null)
  if(!body||!body.category||!categories.has(body.category)) return apiError(422,'validation','Kategorie fehlt.')
