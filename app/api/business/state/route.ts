...
export async function GET(request: Request) {
  ...
  try {
    ...
    return apiJson(record, undefined, id)
  } catch (cause) {
...
export async function PUT(request: Request) {
  ...
  if (!context) return apiError(403, 'tenant_forbidden', 'Keine Berechtigung für diese Organisation.', id)
  if (!body.state || typeof body.state !== 'object' || !Number.isInteger(body.expectedVersion) || Number(body.expectedVersion) < 0) {
  ...
  try {
    ...
    const result = await saveTenantBusinessState({ organizationId: context.organizationId, userId: context.userId, expectedVersion: Number(body.expectedVersion), state })
