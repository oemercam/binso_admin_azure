...
export default function PlatformHelpPage(){
  ...
  const load=useCallback(async()=>{const response=await fetch('/api/platform/help',{cache:'no-store'});if(response.ok)setData(await response.json())},[])
  ...
  async function save(){if(!data.canManage||saving)return;setSaving(true);try{const response=await fetch('/api/platform/help',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({id:selected?.id,slug,title,summary,body,categoryId:categoryId||undefined,keywords:keywords.split(',').map(item=>item.trim()).filter(Boolean),published})});if(response.ok){await load();close()}}finally{setSaving(false)}}

  return <section className="page apple-page">
    <PageHeader title="Help Center" description="Artikel, Kategorien und Hilfreich-Feedback für die digitale Kundenhilfe."/>
    <div className="module-toolbar"><label className="search-field"><Input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Artikel suchen"/></label>{data.canManage?<button type="button" className="button primary" onClick={create}>Neuer Artikel</button>:null}</div>
    <div className="data-list compact-overview-list">{filtered.map(item=><button type="button" className="data-row compact-overview-row" key={item.id} onClick={()=>open(item)}><span className="primary-cell"><strong>{item.title}</strong><small>{item.categoryTitle??'Ohne Kategorie'} · /help/{item.slug}</small></span><span>{item.published?'Publiziert':'Entwurf'}</span><span>{item.helpful} hilfreich · {item.notHelpful} nicht hilfreich</span><span className="row-disclosure">›</span></button>)}</div>
    {(selected||creating)?<div className="settings-section"><h2>{creating?'Neuer Artikel':selected?.title}</h2><label><span>Titel</span><Input value={title} onChange={event=>setTitle(event.target.value)} maxLength={180}/></label><label><span>Slug</span><Input value={slug} onChange={event=>setSlug(event.target.value.toLowerCase())} placeholder="erster-kunde-erfassen"/></label><label><span>Kategorie</span><Select value={categoryId} onChange={event=>setCategoryId(event.target.value)}><option value="">Ohne Kategorie</option>{data.categories.map(category=><option key={category.id} value={category.id}>{category.title}</option>)}</Select></label><label><span>Zusammenfassung</span><Textarea value={summary} onChange={event=>setSummary(event.target.value)} rows={2}/></label><label><span>Inhalt</span><Textarea value={body} onChange={event=>setBody(event.target.value)} rows={10}/></label><label><span>Keywords</span><Input value={keywords} onChange={event=>setKeywords(event.target.value)} placeholder="kunden, erfassen, erste schritte"/></label><label><span>Status</span><Select value={published?'published':'draft'} onChange={event=>setPublished(event.target.value==='published')}><option value="draft">Entwurf</option><option value="published">Publiziert</option></Select></label><div className="module-toolbar"><button type="button" className="button secondary" onClick={close}>Schliessen</button>{data.canManage?<button type="button" className="button primary" disabled={saving} onClick={()=>void save()}>{saving?'Speichern…':'Speichern'}</button>:null}</div></div>:null}
  </section>
}
