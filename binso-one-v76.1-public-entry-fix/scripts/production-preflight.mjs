...
const tenantDatabaseUrl = required('DATABASE_URL')
const platformDatabaseUrl = process.env.PLATFORM_DATABASE_URL?.trim() || ''
if (!platformDatabaseUrl) console.warn('WARN: PLATFORM_DATABASE_URL is not configured. Platform queries still use DATABASE_URL; complete DB-role separation before GA.')
else if (normalizedDatabaseUrl(platformDatabaseUrl) === normalizedDatabaseUrl(tenantDatabaseUrl)) console.warn('WARN: PLATFORM_DATABASE_URL equals DATABASE_URL. Configure separate runtime/platform roles before GA.')
const roleSource = (process.env.PLATFORM_ROLE_SOURCE || 'hybrid').trim().toLowerCase()
if (!['entra','hybrid','database'].includes(roleSource)) fail('PLATFORM_ROLE_SOURCE must be entra, hybrid or database.')
if (roleSource !== 'database') console.warn(`WARN: PLATFORM_ROLE_SOURCE=${roleSource}. Use database after operator assignments are seeded for strict functional-role separation.`)
