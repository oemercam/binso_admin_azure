...
assert.ok(/PLATFORM_DATABASE_URL/.test(read('lib/db/client.ts')) || /serverEnv\.platformDatabaseUrl/.test(read('lib/db/client.ts')))
assert.ok(/PLATFORM_ROLE_SOURCE/.test(read('lib/auth/server.ts')) || /serverEnv\.platformRoleSource/.test(read('lib/auth/server.ts')))
