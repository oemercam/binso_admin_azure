...
for (const file of requiredFiles) {
  if (!fs.existsSync(file)) throw new Error(`Missing V59 authentication foundation file: ${file}`)
}

const onboarding = fs.readFileSync('app/api/onboarding/route.ts', 'utf8')
if (!onboarding.includes("email !== session.user.email.trim().toLowerCase()")) {
  throw new Error('Onboarding must bind the requested owner email to the authenticated identity')
}
...
console.log('Authentication and membership foundation check passed (V59).')
