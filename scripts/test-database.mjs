...
try {
  ...
  await client.query('grant select,insert,update on support_cases,support_messages,in_app_notifications,product_events,organization_milestones,data_lifecycle_requests to binso_v69_rls_test')
  ...
  await client.query("insert into product_events(organization_id,user_id,event_name) values($1,'user-a','customer.created')", [a])
  assert.equal((await client.query('select * from product_events')).rowCount, 1)
  await client.query('savepoint analytics_cross_tenant')
  await assert.rejects(client.query("insert into product_events(organization_id,user_id,event_name) values($1,'user-a','cross-tenant')", [b]), error => error.code === '42501')
  await client.query('rollback to savepoint analytics_cross_tenant')
  await client.query("insert into data_lifecycle_requests(organization_id,requested_by_user_id,request_type) values($1,'user-a','export')", [a])
  assert.equal((await client.query('select * from data_lifecycle_requests')).rowCount, 1)
  ...
  console.log('PostgreSQL integration checks passed: tenant RLS, support/analytics/lifecycle WITH CHECK, case numbering and pending document deduplication.')
} finally {
