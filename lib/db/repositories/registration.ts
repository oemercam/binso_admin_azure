...
import type { SignupMode, SignupRequest, SubscriptionPlan } from '@/types/domain'
...
const signupColumns = `id, company_name, owner_name, email, plan, signup_mode, status, created_at, updated_at, organization_id, onboarding_status, onboarding_step, onboarding_completed_steps, onboarding_module_preferences, onboarding_business_settings`
...
export async function saveRegistration(input: {
  ...
  const result = await query<SignupRow>(
    `insert into signup_requests (company_name, owner_name, email, plan, signup_mode, status, user_id, updated_at)
     values ($1, $2, $3, $4, $5, 'account_created', $6, now())
    ...
    [input.companyName.trim(), input.ownerName.trim(), input.email.trim().toLowerCase(), input.plan, input.mode, input.userId],
  )
