...
alter table organization_subscriptions add column if not exists billing_provider text not null default 'manual' check (billing_provider in ('manual','stripe'));
alter table organization_subscriptions add column if not exists billing_subscription_id text;
alter table organization_subscriptions add column if not exists billing_interval text not null default 'monthly' check (billing_interval in ('monthly','yearly'));
...
alter table organization_subscriptions add column if not exists scheduled_plan text check (scheduled_plan is null or scheduled_plan in ('starter','business','professional','enterprise'));
...
create unique index if not exists uq_org_subscriptions_billing_customer on organization_subscriptions(billing_customer_id) where billing_customer_id is not null;
create index if not exists idx_org_subscriptions_billing_sync on organization_subscriptions(billing_provider, billing_last_synced_at desc);
create index if not exists idx_billing_webhook_org_received on billing_webhook_events(organization_id, received_at desc);
